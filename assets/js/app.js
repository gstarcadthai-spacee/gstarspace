...
let products = PRODUCT_DEFAULTS.map(product => ({ ...product }));
...
function normalizeCategory(value = "") {
  ...
  const map = {
    ...
    "boq-estimation": "boq-estimation",
    "viewer & collaboration": "viewer-collaboration",
    "viewer-collaboration": "viewer-collaboration",
    "add-ons": "addons",
    "addons": "addons",
  ...
  return map[key] || key.replace(/\s+/g, "-") || "partners";
}

function legacyByIdOrName(row) {
  const id = String(row.ID || row.id || "").toLowerCase();
  const name = String(row.Name || row.name || "").toLowerCase();
  return PRODUCT_DEFAULTS.find(item =>
    item.id.toLowerCase() === id || item.name.toLowerCase() === name
  );
}

function mapApiProduct(row) {
  const fallback = legacyByIdOrName(row);
  const officialWebsite = row.WebsiteURL || fallback?.overview?.["Official Website"] || "#";
  const downloadURL = row.DownloadURL || officialWebsite;
  const openTicketURL = row.OpenTicketURL || "https://cs.applicadthai.com/";
  const lineURL = fallback?.actions?.find(action => action.title === "LINE Hotline")?.url || "https://line.me/";

  return {
    id: row.ID || fallback?.id || crypto.randomUUID(),
    name: row.Name || fallback?.name || "Untitled Product",
    category: normalizeCategory(row.Category || fallback?.category),
    logo: row.Logo || fallback?.logo || "",
    tagline: row.Tagline || row.Description || fallback?.tagline || "",
    status: row.Status || "Active",
    sort: Number(row.Sort || 999),
    overview: {
      "Current Version": row.CurrentVersion || fallback?.overview?.["Current Version"] || "Latest",
      "Developer": row.Developer || fallback?.overview?.Developer || "",
      "Platform": row.Platform || fallback?.overview?.Platform || "",
      "License": row.License || fallback?.overview?.License || "",
      "Official Website": officialWebsite
    },
    actions: [
      { icon: "📥", title: "Download Trial", url: downloadURL },
      { icon: "📥", title: "Download Trial + Script", url: downloadURL },
      { icon: "💬", title: "Open Ticket", url: openTicketURL },
      { icon: "🟢", title: "LINE Hotline", url: lineURL }
    ]
  };
}

async function loadProductsFromAPI() {
  if (typeof GstarAPI === "undefined") {
    console.warn("GstarAPI not loaded. Using built-in product backup.");
    return PRODUCT_DEFAULTS.map(product => ({ ...product }));
  }

  try {
    const rows = await GstarAPI.list("products");
    if (!Array.isArray(rows) || rows.length === 0) {
      console.warn("Products sheet is empty. Using built-in product backup.");
      return PRODUCT_DEFAULTS.map(product => ({ ...product }));
    }

    return rows
      .filter(row => String(row.Status || "Active").toLowerCase() !== "hidden")
      .map(mapApiProduct)
      .sort((a, b) => a.sort - b.sort);
  } catch (error) {
    console.error("Could not load products from API. Using backup.", error);
    return PRODUCT_DEFAULTS.map(product => ({ ...product }));
  }
}
...
if (filterRoot && grid && searchInput) {
  bootProducts();
}
